...
for (const rel of files) {
  ...
  const start = text.indexOf(brokenPrefix);
  const end = text.indexOf(brokenSuffix, start);
  if (start >= 0 && end >= 0) {
    const after = end + brokenSuffix.length;
    text = text.slice(0, start) + expectedImport + text.slice(after);
